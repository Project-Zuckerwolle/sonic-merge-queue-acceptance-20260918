    ...
    def _bash(self, args: dict) -> dict:
        ...
        try:
            proc = subprocess.run(
                ...
                timeout=timeout, cwd=str(self.ws.root, creationflags=_NO_WINDOW),
            )
            return {
    ...
    def _powershell(self, args: dict) -> dict:
        ...
        try:
            proc = subprocess.run(
                ...
                capture_output=True, encoding="utf-8", text=True, timeout=timeout, cwd=str(self.ws.root, creationflags=_NO_WINDOW),
            )
