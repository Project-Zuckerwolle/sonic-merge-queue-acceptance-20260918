...
// Phase 4 (C1 Umbau): NOVA_PINNED_STREAM. Wenn gesetzt, werden die gestreamten 24B-Host-Puffer
// (packed + Skalen) einmalig via cudaHostRegister in-place gepinnt -> H2D nutzt DMA (pinned, ~1,5x).
// Kein Umbau der Speicherhaltung, kein Extra-RAM. Schlägt das Registrieren fehl (Aggregat-Pin-Limit),
// bleibt der Puffer pageable (Fehler wird verworfen) -> graceful degradation.
